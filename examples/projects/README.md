...
A role may carry an `identity`, naming a directory under `<config>/identities/`. Every task
dispatched for the role is then performed under it: the agent gets that identity's own memory —
the same one every previous run of the role had, so a reviewer remembers what it has already asked
for — its `AGENTS.md` as standing instructions, and, where the identity holds a taxis token,
comments and reviews recorded as coming from that identity instead of from orchestra.

The identity is *who* the agent is; the role is what it does. Where that line falls in practice:
the prompt template belongs to the role, because it renders this issue; the standing instructions
belong to the identity, because they hold whatever the issue is. See "identities" in the top-level
README.
