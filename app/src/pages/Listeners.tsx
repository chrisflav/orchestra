...
import { EnableListener } from "../components/EnableListener";
import { EnabledStatus } from "../components/Status";
import { Time } from "../components/Time";
import { interval, orDash } from "../core/format";
    ...
    >
      {(data) => {
        const facts: Fact[] = [
          {
            key: "State",
            // The switch sits with the fact it changes rather than in a toolbar, the same
            // place — and for the same reason — as Cancel on a task page.
            value: (
              <span className="fact-inline">
                <EnabledStatus enabled={data.enabled} />
                <EnableListener name={data.name} enabled={data.enabled} />
              </span>
            ),
          },
