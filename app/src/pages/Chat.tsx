...
function NewSession() {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    setBusy(true);
    setError(null);
    startSession(backend.id, { upstream: upstream.trim(), fork: fork.trim() })
...
function Compose({ session, onEnded }: { session: SessionDetail; onEnded: () => void }) {
  ...
  const submit = (e: React.FormEvent) => {
    ...
    setBusy(true);
    setError(null);
    sendTurn(backend.id, session.id, turn)
      .catch((err: unknown) => {
        ...
        <div className="chat-actions">
          ...
          {canInterrupt && (
            <button
              type="button"
              disabled={busy}
              onClick={() => act(interruptSession(backend.id, session.id), () => undefined)}
            >
              Interrupt
            </button>
          )}
            ...
            <button
              ...
              onClick={() => {
                if (window.confirm("End this session? The transcript is kept.")) {
                  act(endSession(backend.id, session.id), onEnded);
                }
              }}
            >
              End session
