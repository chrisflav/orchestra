...
(`orchestra dashboard generate`), so they can be served by any HTTP server. Each
page fetches its data as JSON from the `/api/...` endpoints of the backend
started with `orchestra dashboard serve`. The same JSON is streamed over
