...
(the usage endpoint is rate-limiting requests)"
      ...
      for b in st.blocks do
        if Usage.blockIsLive b now then
          ...
          let lifts :=
            if b.notAWindow then " — no reset will clear this"
            else match b.untilEpoch with
              | some u => s!", lifts {Usage.relativeToNow u now}"
              | none   => ""
          let alsoUnnamed := if b.coversUnscoped then " (also holds tasks that name no model)" else ""
          IO.println s!"    observed ({scope}): {b.reason}{lifts}{alsoUnnamed}"
